...
const editReserva = async (mes_reserva, tipo_reserva, clinte_id, id) => {
    const result = await pool.query (`UPDATE reservas SET mes_reserva = $1, tipo_reserva = $2, cliente_id = $3 WHERE id = $4 RETURNING *`, [mes_reserva, tipo_reserva, clinte_id, id])
    return result.rows[0]
}
